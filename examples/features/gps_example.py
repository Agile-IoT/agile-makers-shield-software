...
def run_example():
   ...
   while(True):
      ...
      print("\tUTC: {:02d}:{:02d}:{:02d}".format(gga_utc["h"], gga_utc["m"], gga_utc["s"]))
      print("\tFix quality: {}; Number of satellites: {}".format(gga["fix"], gga["nsat"]))
      print("\tLatitude / Longitude: {}{} / {}{}".format(gga["latitude"], gga["latitudeDir"], gga["longitude"], gga["longitudeDir"]))
      print("\tAltitude: {}{}".format(gga["altitude"], gga["altitudeUnit"]))
      print("\tHorizontal dilution of position: {}".format(gga["hdop"]))
      print("\tHeight of geoid: {}{}".format(gga["hog"], gga["hogUnit"]))
      ...
      print("\tUTC: {:02d}:{:02d}:{:02d}".format(rmc_utc["h"], rmc_utc["m"], rmc_utc["s"]))
      print("\tDate: {:04d}/{:02d}/{:02d}".format(rmc_date["y"], rmc_date["m"], rmc_date["d"]))
      print("\tFix: {}; Status: {}".format(rmc["fix"], rmc["status"]))
      print("\tLatitude / Longitude: {}{} / {}{}".format(rmc["latitude"], rmc["latitudeDir"], rmc["longitude"], rmc["longitudeDir"]))
      print("\tSpeed over the ground in knots: {}".format(rmc["spkn"]))
      print("\tTrack angle in degrees: {}".format(rmc["angle"]))
      print("\tMagnetic variation: {}{}".format(rmc["magneticVar"], rmc["magneticVarDir"]))
      print()
