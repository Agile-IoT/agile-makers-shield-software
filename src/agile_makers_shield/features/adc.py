...
class ADC_Obj(dbF.FeatureObj):
   ...
   def readADC(self, args):
      ...
      try:
         ...
         self._logger.debug("{}@readADC: Channel not specified".format(self._full_path))
         raise ADC_Exception("Channel not specified.")
      if not channel in ADC_CHANNELS.keys():
         self._logger.debug("{}@readADC: Invalid channel".format(self._full_path))
         raise ADC_Exception("Invalid channel.")
      mode = args.pop(ADC_PARAMS["MODE"], ADC_DEFAULTS["MODE"])
      if not mode in ADC_MODES.keys():
         self._logger.debug("{}@readADC: Invalid mode".format(self._full_path))
         raise ADC_Exception("Invalid mode.")
      resolution = args.pop(ADC_PARAMS["RESOLUTION"], ADC_DEFAULTS["RESOLUTION"])
      if not resolution in ADC_RESOLUTIONS.keys():
         self._logger.debug("{}@readADC: Invalid resolution".format(self._full_path))
         raise ADC_Exception("Invalid resolution.")
      pga = args.pop(ADC_PARAMS["PGA"], ADC_DEFAULTS["PGA"])
      if not pga in ADC_PGAS.keys():
         self._logger.debug("{}@readADC: Invalid pga".format(self._full_path))
      ...
      try:
         ...
         self._logger.debug("{}@readADC: Problem reading from the AGILE Maker's Shield".format(self._full_path))
         raise ADC_Exception("Problem reading from the AGILE Maker's Shield.")
      result = {}
