...
class XBee_802_15_4_Obj(dbP.ProtocolObj):
   ...
   def Connect(self):
      ...
      # self._module = xbee.XBee(self._serial, escaped=self._setup[APIMODE2], callback=self._update_data)
      ...
      for option in self._setup[ATCMDS]:
         ...
         time.sleep(GUARDTIME)
         self._logger.debug("{}@Connect: Sending AT command={}, parameter={}".format(self._full_path, cmdEnc, paramEnc))
         self._module.send("at", frame_id=b"R", command=cmdEnc, parameter=paramEnc)
         rx = self._get_module_data()
         if not rx or not rx["status"]:
            self._logger.debug("{}@Connect: Did not receive response from AT command".format(self._full_path))
            self._serial.close()
            raise XBee_802_15_4_Exception("Did not receive response from AT command")
         if rx["status"] != b"\x00":
            self._logger.debug("{}@Connect: Wrong AT command/parameter ({}/{})".format(self._full_path, cmd, param))
            self._serial.close()
            raise XBee_802_15_4_Exception("Wrong AT command/parameter ({}/{})".format(cmd, param))
      if writeChanges:
         self._logger.debug("{}@Connect: Writting changes".format(self._full_path))
   ...
   def Send(self, args):
      ...
      if not cmd in APITXCMDS:
         self._logger.debug("{}@Send: A valid API command must be provided".format(self._full_path))
         raise XBee_802_15_4_Exception("A valid API command must be provided {}.".format(APITXCMDS))
      params = {}
      for key in args.keys():
         if type(args[key]) == dbus.Array:
            params[key] = bytes(args[key])
      self._logger.debug("{}@Send: Sending {} with params {}".format(self._full_path, cmd, params))
   ...
   def Receive(self):
      ...
      self._logger.debug("{}@Receive: Received {}".format(self._full_path, result))
