...
class ATMega(metaclass=singleton.Singleton):
    """Read from and write to the AT Mega via I2C."""
    ...
    @lock_decorator
    def _check(self):
        """Check if the I2C bus is working."""
        reg = ATMEGA_CHECK
        try:
    ...
    def _getGPSBufferSize(self):
        """Return the size of the GPS buffer in the ATMega."""
    ...
    def uartON(self, socket, baudrate=UART_BAUD_9600, databits=UART_DATABITS_8,
        ...
        """Turn on the UART of the specified socket."""
    ...
    def uartOFF(self, socket):
        """Turn off the UART of the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
        reg = (socket << SOCKET_SHIFT) | SOCKET_STATUS
    ...
    def uartStatus(self, socket):
        """Return the status of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
        reg = (socket << SOCKET_SHIFT) | SOCKET_STATUS
        return self._bus.read(reg, 1)[0]
    ...
    def sendData(self, socket, data):
        """Send an array of bytes to the UART of the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def getData(self, socket):
        """Read all the available bytes of the UART of the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def getBaudrate(self, socket):
        """Return the baudrate of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
        ...
        if len(baud) != 4:
            return 0
        baudrate = (baud[0] << 32) | (baud[1] << 16) | \
                   (baud[2] << 8) | (baud[3])
        return baudrate

    @lock_decorator
    def _setBaudrate(self, socket, baudrate):
        """Set the baudrate of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def getDatabits(self, socket):
        """Return the databits of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def _setDatabits(self, socket, databits):
        """Set the databits of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
        if databits not in DATABITS:
    ...
    def getStopbits(self, socket):
        """Return the stopbits of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def _setStopbits(self, socket, stopbits):
        """Set the stopbits of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def getParity(self, socket):
        """Return the parity of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def _setParity(self, socket, parity):
        """Set the parity of the UART in the specified socket."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def getUartInterrupt(self, socket):
        """Return if the UART of the specified socket has new data."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    def getButtonInterrupt(self, socket):
        """Return if the button of the specified socket was pressed."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket")
    ...
    @lock_decorator
    def updateGPS(self):
        """Update the GGA and RMC buffers in the ATMega with the last data."""
    ...
    def setLedSocket(self, socket, rgb_bright):
        """Set the RGB brightness of the specified socket LED."""
        if socket not in SOCKETS:
            raise ValueError("Wrong socket LED")
        if len(rgb_bright) != 3:
    ...
    def getLedAux(self, aux):
        """Return the brightness of the specified auxiliar LED."""
        if aux not in LEDS_AUX:
    ...
    def setLedAux(self, aux, bright):
        """Set the brightness of the specified auxiliar LED."""
        if aux not in LEDS_AUX:
            raise ValueError("Wrong auxiliar LED")
        bright = bright & 0xFF
        reg = (SOCKET_LEDS << SOCKET_SHIFT) | LEDS_AUX[aux]
