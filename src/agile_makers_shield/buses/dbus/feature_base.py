...
class FeatureException(dbus.DBusException):
   ...
   def __init__(self, feature_name, msg=""):
      if msg == "":
         ...
         super().__init__(msg)
      self._dbus_error_name = db_cons.BUS_NAME["Feature"] + "." + feature_name
...
class FeatureObj(dbus.service.Object):
   ...
   @dbus.service.method(db_cons.BUS_NAME["Feature"], in_signature="", out_signature="s")
   def Name(self):
...
class Feature():
   ...
   def __init__(self):
      self._name = dbus.service.BusName(db_cons.BUS_NAME["Feature"], dbus.SessionBus())
# -----------------------
